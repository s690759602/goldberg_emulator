...
const char *get_steam_path() {
  ...
  // Allocate more space for steam_path if needed (required_size does not count
  // terminator)
  if (required_size > 0 && required_size >= STEAM_PATH_CACHE_SIZE) {
    char *large_steam_path = (char *)malloc(sizeof(char) * (required_size + 1));
    int check_size = snprintf(large_steam_path, required_size + 1,
                              "%s/.steam/steam", home_path);
...
bool match_path(char *path, int start, bool accept_same_case) {
  ...
  if (!path_accessible ||
      (!is_last_component && !match_path(path, separator, accept_same_case))) {
        ...
        if (!strcasecmp(&path[component], entry_name)) {
          ...
          if (case_differs) {
            ...
            if (is_last_component ||
                match_path(path, separator, accept_same_case)) {
...
const char *lowercase_path(const char *path, bool accept_same_case,
                           bool stop_at_separator) {
  std::locale loc;
  ...
  if (path && *path) {
    ...
    if (access(path, F_OK)) {
      ...
      while ((!stop_at_separator || *iterator != PATH_SEPARATOR_CHAR) &&
             *iterator) {
...
STEAMAPI_API FILE *__wrap_freopen(const char *path, const char *modes,
                                  FILE *stream) {
...
STEAMAPI_API int __wrap___xstat(int ver, const char *path,
...
STEAMAPI_API int __wrap___lxstat(int ver, const char *path,
                                 struct stat *stat_buf) {
  const char *path_lowercased = lowercase_path(path, false, false);
#ifdef __APPLE__
...
STEAMAPI_API int __wrap_scandir(const char *path, struct dirent ***namelist,
...
STEAMAPI_API int __wrap_scandir64(const char *path, struct dirent64 ***namelist,
...
#ifndef __APPLE__
STEAMAPI_API int __wrap___xstat64(int ver, const char *path,
...
#ifndef __APPLE__
STEAMAPI_API int __wrap___lxstat64(int ver, const char *path,
                                   struct stat64 *stat_buf) {
  const char *path_lowercased = lowercase_path(path, false, false);
...
STEAMAPI_API int __wrap_mount(const char *source, const char *target,
...
#ifdef __APPLE__
  int result =
      mount(source_lowercased, target_lowercased, mountflags, (void *)data);
#else
  int result = mount(source_lowercased, target_lowercased, filesystemtype,
                     mountflags, data);
#endif
...
STEAMAPI_API int __wrap_utimes(const char *path,
